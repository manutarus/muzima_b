...
public class FingerPrintActivity
    ...
    public void setFingerPrintData(String encodeBytes) {
        ...
        try {
            ObsInterface.fingerprintResultBytes = encodeBytes.getBytes("UTF-8");
        ...
        try {
            ...
            for (Patient patient : patients) {
                ...
                for (PersonAttribute personAttribute1 : personAttribute) {
                    if (personAttribute1.getAttributeType().getName().equals("fingerprint")) {
                        patientRecord = new PatientFingerPrints();
                        patientRecord.setTemplate(Base64.decode(patient.getAttribute("fingerprint").getAttribute(), Base64.DEFAULT));
            ...
            Log.w(TAG, "Exception occurred while fetching patients", e.getCause());
        }
