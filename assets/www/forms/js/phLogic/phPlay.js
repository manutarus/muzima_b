$(document).ready(function () {
    ...
    $('#out_standing_balance').val($.fn.currentBalanceBF());
    if($('#out_standing_balance').val() == ''){
        $('#outstanding_balance').hide();
        $('#new_balance').removeClass('hidden');
    }

    /* Start - Compute the expected pay */
    var $outstandingBal = $('#out_standing_balance');
    var $newOutstandingBal = $('#new_out_standing_balance');
    var $currentBill = $('#current_bill');
    var $expectedPay = $('.expectedPay');
    var displayExpectedPay = function () {
        if (($currentBill.val() != '') && (($outstandingBal.val() != '')||($newOutstandingBal.val() != ''))) {
            $expectedPay.find('input').val(computeExpectedPay());
            $expectedPay.show();
        } else {
            $expectedPay.hide();
        }
    };
    var computeExpectedPay = function () {
        if($newOutstandingBal.val() != ''){
            return parseInt($newOutstandingBal.val()) + parseInt($currentBill.val());
        }else{
            return parseInt($outstandingBal.val()) + parseInt($currentBill.val());
        }
    };

    $outstandingBal.change(displayExpectedPay);
    $outstandingBal.trigger('change');

    $newOutstandingBal.change(displayExpectedPay);
    $newOutstandingBal.trigger('change');

    $currentBill.change(displayExpectedPay);
    $currentBill.trigger('change');

    /* Start - Compute Balance to carry forward */
    var $expected_pay = $('#expected_pay');
    var $payment_relieved = $('#payment_relieved');
    var $balance_cf = $('.balance_cf');
    var displayBalanceCf = function () {
        if (($expected_pay.val() != '') && ($payment_relieved.val() != '')) {
            $balance_cf.find('input').val(balanceToCF());
            $balance_cf.show();
        } else {
            $balance_cf.hide();
        }
    };
    var balanceToCF = function () {
        return parseInt($expected_pay.val())- parseInt($payment_relieved.val());
    };

    $expected_pay.change(displayBalanceCf);
    $expected_pay.trigger('change');

    $payment_relieved.change(displayBalanceCf);
    $payment_relieved.trigger('change');
